...
import { MedProductItem } from './MedProductItem';

export const MedSubProduct = ({ category, subcategory }) => {
	const { medicineProducts } = useContext(ShopContext);
	const [related, setRelated] = useState([]);

	useEffect(() => {
		if (medicineProducts.length > 0) {
			const filteredProducts = medicineProducts.filter(
				(item) => item.category === category && item.subcategory === subcategory
			);
			setRelated(filteredProducts);
		}
	}, [medicineProducts, category, subcategory]);
